...
mod tests {
    ...
    fn test_script_default_compact() {
        ...
        assert_tokens(
            &s.compact(),
            &[
                Token::BorrowedBytes(&[10, 0, 1, 47, 0]),
            ],
        );
    }
    ...
    fn test_script_default_cbor() {
        ...
        assert_eq!(b, vec![69, 10, 0, 1, 47, 0]);
    ...
    fn test_script_cidv0_compact() {
        ...
        assert_tokens(
            &s.compact(),
            &[
                Token::BorrowedBytes(&[10, 2, 1, 47, 18, 32, 226, 140, 122, 235, 58, 135, 107, 37, 237, 130, 36, 114, 228, 122, 105, 111, 226, 82, 20, 193, 103, 47, 9, 114, 25, 95, 155, 100, 238, 164, 30, 126])
            ]
            ,
        );
    }
    ...
    fn test_script_cidv0_cbor() {
        ...
        let s = Script::Cid(Key::default(), v0);
        let b = serde_cbor::to_vec(&s).unwrap();
        assert_eq!(b, vec![88, 38, 10, 2, 1, 47, 18, 32, 226, 140, 122, 235, 58, 135, 107, 37, 237, 130, 36, 114, 228, 122, 105, 111, 226, 82, 20, 193, 103, 47, 9, 114, 25, 95, 155, 100, 238, 164, 30, 126]);
        assert_eq!(s, serde_cbor::from_slice(b.as_slice()).unwrap());
    }
    ...
    fn test_script_cidv1_compact() {
        ...
        assert_tokens(
            &s.compact(),
            &[
                Token::BorrowedBytes(&[10, 2, 1, 47, 1, 113, 20, 64, 87, 146, 218, 217, 96, 133, 182, 7, 107, 142, 78, 99, 181, 120, 201, 13, 3, 54, 188, 170, 222, 244, 242, 71, 4, 223, 134, 97, 73, 82, 106, 30, 109, 35, 248, 158, 33, 138, 211, 246, 23, 42, 126, 38, 230, 227, 122, 61, 234, 114, 142, 95, 35, 46, 65, 105, 106, 210, 134, 188, 202, 146, 1, 190]),
            ],
        );
    }
    ...
    fn test_script_cidv1_cbor() {
        ...
        let s = Script::Cid(Key::default(), v1);
        let b = serde_cbor::to_vec(&s).unwrap();
        assert_eq!(b, vec![88, 72, 10, 2, 1, 47, 1, 113, 20, 64, 87, 146, 218, 217, 96, 133, 182, 7, 107, 142, 78, 99, 181, 120, 201, 13, 3, 54, 188, 170, 222, 244, 242, 71, 4, 223, 134, 97, 73, 82, 106, 30, 109, 35, 248, 158, 33, 138, 211, 246, 23, 42, 126, 38, 230, 227, 122, 61, 234, 114, 142, 95, 35, 46, 65, 105, 106, 210, 134, 188, 202, 146, 1, 190]);
        assert_eq!(s, serde_cbor::from_slice(b.as_slice()).unwrap());
    }
